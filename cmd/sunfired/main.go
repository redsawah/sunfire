...
func main() {
	...
	case *update, *checkUpdate:
		...
		return
	case *setup:
		if err := menu.Server(*cfgPath); err != nil {
			log.Fatal(err)
		}
		return
	case *top:
...
func run(cfgPath string) error {
	...
	// мягкая перезагрузка: добавить пользователя можно без разрыва соединений
	...
	for name, addr := range cfg.Listen {
		...
		ln, err := t.Listen(addr, cfg.TransportKey)
		if err != nil {
...
// live — конфиг, который можно подменить на ходу. Новые соединения берут
// свежий снимок, уже установленные продолжают работать как работали.
type live struct {
	mu     sync.RWMutex
	cfg    config.Config
...
func (l *live) get() (config.Config, map[string]string) {
	l.mu.RLock()
	defer l.mu.RUnlock()
	return l.cfg, l.tokens
}

func (l *live) set(c config.Config) {
	l.mu.Lock()
	l.cfg, l.tokens = c, c.Tokens()
	l.mu.Unlock()
}
...
// reload перечитывает конфиг по сигналу. Адреса прослушивания и транспортный
// ключ на ходу не меняются — для них нужен рестарт.
func reload(path string) {
	c, err := config.Load(path)
	if err != nil {
		log.Printf("перезагрузка конфига: %v", err)
		return
	}
	old, _ := cur.get()
	if !sameListen(old.Listen, c.Listen) || old.TransportKey != c.TransportKey || old.StatusAddr != c.StatusAddr {
		log.Printf("перезагрузка: адреса, статус и транспортный ключ применятся только после рестарта")
	}
	cur.set(c)
	log.Printf("конфиг перечитан, пользователей: %d", len(c.Tokens()))
}

func sameListen(a, b map[string]string) bool {
	if len(a) != len(b) {
		return false
	}
	for k, v := range a {
...
func handle(raw net.Conn, cfg config.Config, tokens map[string]string) {
	...
	// место занимает машина, а не соединение; держим до конца этого соединения
	limit := cfg.Limit(user)
	host := ""
	if ip != nil {
		host = ip.String()
	}
	if !seats.acquire(user, host, limit) {
		log.Printf("отказ %s: занято мест %d", user, limit)
		return
	}
	defer func() {
		if !keep {
			seats.release(user, host, limit)
		}
	}()
	...
	case stripe.KindStart:
		...
		conns, ok := sess.start(head.SID, user, host, limit, head.Target, head.Count, conn)
		if !ok {
			return
		}
		keep = true
	...
	case stripe.KindJoin:
		conns, target, ok := sess.join(head.SID, user, host, limit, head.Index, conn)
		if !ok {
			return
		}
		keep = true
...
type pending struct {
	user   string
	ip     string // адрес машины: места держит сессия, а не отдельное соединение
	limit  int
	target string
...
func (s *sessions) at(sid, user, ip string, limit int) *pending {
	p := s.m[sid]
	if p == nil {
		p = &pending{user: user, ip: ip, limit: limit, conns: make([]io.ReadWriteCloser, maxStripe)}
...
func (s *sessions) start(sid, user, ip string, limit int, target string, n int, c io.ReadWriteCloser) ([]io.ReadWriteCloser, bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
	p := s.at(sid, user, ip, limit)
...
func (s *sessions) join(sid, user, ip string, limit int, idx int, c io.ReadWriteCloser) ([]io.ReadWriteCloser, string, bool) {
	if idx >= maxStripe {
		return nil, "", false
	}
	s.mu.Lock()
...
func (s *sessions) drop(sid string, p *pending) {
	...
	for i := 0; i < p.got; i++ {
		seats.release(p.user, p.ip, p.limit)
	}
	log.Printf("раскладка %s не собралась", sid)
}

// clients следит, сколько разных машин работает под одним токеном.
// Считаем адреса, а не соединения: машина с десятком сессий — одно место.
type clients struct {
	mu sync.Mutex
	m  map[string]map[string]int
}

var seats = &clients{m: map[string]map[string]int{}}

func (c *clients) acquire(user, ip string, limit int) bool {
	if limit <= 0 || ip == "" {
		return true
	}
	c.mu.Lock()
	...
	if _, ok := byIP[ip]; !ok && len(byIP) >= limit {
		return false
	}
	byIP[ip]++
	return true
...
func (c *clients) release(user, ip string, limit int) {
	if limit <= 0 || ip == "" {
		return
	}
	c.mu.Lock()
