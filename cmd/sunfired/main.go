...
func handle(raw net.Conn, cfg config.Config, tokens map[string]string) {
	...
	case stripe.KindJoin:
		...
		keep = true
		if conns != nil {
			serveStripe(conns, target)
		}

	default:
		serveOne(halfConn{Conn: conn, raw: raw}, head.Target)
	}
}
...
func serveOne(conn io.ReadWriteCloser, target string) {
...
func serveStripe(conns []io.ReadWriteCloser, target string) {
	s := stripe.Join(conns)
	defer s.Close()
	serveOne(s, target)
}
