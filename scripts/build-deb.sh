...
[Service]
EnvironmentFile=/etc/sunfire/client.env
ExecStart=/usr/bin/sunfire -gw ${GW} -token ${TOKEN} -socks ${SOCKS}
Restart=always
RestartSec=2
NoNewPrivileges=yes
ProtectSystem=strict
...
for arch in $ARCHS; do
	...
	cat > "$d/etc/sunfire/client.env.example" <<'EOF'
# адрес шлюза host:port
GW=
# токен, выданный шлюзом
TOKEN=
SOCKS=127.0.0.1:1080
EOF
	chmod 755 "$d/usr/bin/sunfire"
	chmod 644 "$d/lib/systemd/system/sunfire.service" "$d/etc/sunfire/client.env.example"
	...
	cat > "$d/etc/sunfire/main.conf.example" <<'EOF'
{
  ...
  "users": [
    { "name": "node-a", "token": "СЮДА ТОКЕН ОТ sunfired -token", "allow_ips": ["203.0.113.7"] }
  ]
}
EOF
