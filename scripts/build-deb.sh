	...
	cat > "$d/etc/sunfire/client.conf.example" <<'EOF'
{
  ...
  "streams": 1,
  "servers": [
    { "name": "node-a", "gw": "СЕРВЕР:9000", "token": "СЮДА ТОКЕН ОТ sunfired -token", "transport_key": "СЮДА ТРАНСПОРТНЫЙ КЛЮЧ ШЛЮЗА, ПУСТО — ТОЛЬКО TCP" }
  ]
}
EOF
	...
	cat > "$d/etc/sunfire/main.conf.example" <<'EOF'
{
  ...
  "transport_key": "СЮДА ТРАНСПОРТНЫЙ КЛЮЧ ИЗ sunfired -menu",
  "users": [
