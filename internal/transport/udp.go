...
func (udpTransport) Dial(addr, key string, _ time.Duration) (net.Conn, error) {
...
func (udpTransport) Listen(addr, key string) (net.Listener, error) {
	aead, err := deriveAEAD(key)
	if err != nil {
		return nil, err
	}
...
func deriveAEAD(key string) (cipher.AEAD, error) {
	r := hkdf.New(sha256.New, []byte(key), nil, []byte("sunfire/1 udp"))
