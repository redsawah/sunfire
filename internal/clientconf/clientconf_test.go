...
func TestSaveLoad(t *testing.T) {
	...
	c.Servers = []clientconf.Server{
		{Name: "ams", GW: "10.0.0.1:9000", Token: token, TransportKey: key},
...
func TestLoadOtkaz(t *testing.T) {
	cases := []struct {
		...
		{"короткий транспортный ключ", `{"servers":[{"name":"a","gw":"h:1","token":"` + token + `","transport_key":"коротко"}]}`},
...
// Транспортный ключ необязателен: без него сервер доступен только по tcp.
func TestTransportKey(t *testing.T) {
	path := write(t, "client.conf", `{"servers":[
		{"name":"a","gw":"h:1","token":"`+token+`"},
		{"name":"b","gw":"h:2","token":"`+token+`","transport_key":" `+key+` "}
	]}`)
	c, err := clientconf.Load(path)
	if err != nil {
		t.Fatal(err)
	}
	if c.Servers[0].TransportKey != "" {
		t.Fatalf("ключ взялся ниоткуда: %q", c.Servers[0].TransportKey)
	}
	if c.Servers[1].TransportKey != key {
		t.Fatalf("ключ %q", c.Servers[1].TransportKey)
	}
}
