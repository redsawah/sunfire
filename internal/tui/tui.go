...
func (m *meter) frame(s stats.Snapshot) string {
	...
	// раскладка тянется на весь экран: считаем, сколько строк занимает
	// постоянная часть, остаток делим между графиками, таблицей и журналом
	head := 3
	if s.Extra != "" {
		head++
	}
	cores := len(coreLoad(m.cores))
	fixed := head + 2 /* заголовки графиков */ + 3 /* CPU, ядра, ОЗУ */ + 2 /* заголовок таблицы */ + 2 /* журнал */ + 1 /* низ */
	if cores == 0 {
		fixed--
	}
	free := h - fixed
	...
	// графики забирают до половины остатка, поровну на загрузку и отдачу
	grows := free / 4
	if grows > 10 {
		grows = 10
	}
	...
	b.WriteString(fmt.Sprintf("\033[1mсессий\033[0m %d \033[2m(всего %d)\033[0m   \033[36m↓ %s\033[0m   \033[35m↑ %s\033[0m   \033[2mпринято\033[0m %s \033[2mотдано\033[0m %s\n",
		conns, total, bps(dn), bps(up), size2(sumDn), size2(sumUp)))
	...
	m.cores = readCores()
	if len(m.lastCore) > 0 {
		b.WriteString(coreBar(m.lastCore, w))
	}
