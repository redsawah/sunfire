...
func (m *meter) frame(s stats.Snapshot) string {
	...
	// четыре панели: шапка с системой, канал, сессии, журнал
	sys := sysLines(m, inner)
	fixed := 4*2 /* рамки */ + 2 /* шапка */ + len(sys) + 2 /* подписи канала */ + 1 /* заголовок таблицы */ + 1 /* низ */
	free := h - fixed
	if free < 8 {
		free = 8
	}
	...
	head = append(head, hl)
	head = append(head, sys...) // нагрузка машины — в тот же верхний блок
	b.WriteString(panel(fmt.Sprintf("sunfire · %s %s · работает %s", s.Role, s.Version,
		dur(time.Duration(s.Uptime)*time.Second)), head, w))

	// загрузка и отдача — одним блоком
	ch := make([]string, 0, 2*grows+2)
	ch = append(ch, fmt.Sprintf(" \033[36m▼ загрузка\033[0m %s   \033[2mпик %s\033[0m", bps(dn), bps(peakOf(m.histDn))))
	ch = append(ch, graphLines(m.histDn, grows, inner, "\033[36m")...)
	ch = append(ch, fmt.Sprintf(" \033[35m▲ отдача\033[0m %s   \033[2mпик %s\033[0m", bps(up), bps(peakOf(m.histUp))))
	ch = append(ch, graphLines(m.histUp, grows, inner, "\033[35m")...)
	b.WriteString(panel("канал", ch, w))
