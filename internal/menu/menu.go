...
func defaultServer() config.Config {
	...
	// без транспортного ключа udp не поднимется; не вышло — впишут из меню
	c.TransportKey, _ = config.NewTransportKey()
	return c
}
...
func (m *serverMenu) screen() {
	...
	m.printf("  3. Транспортный ключ: %s\n", orUnset(mask(m.cfg.TransportKey)))
	m.printf("  4. Привязка к адресу клиента: %s\n", onOff(m.cfg.PinClientIP))
	m.print("  5. Монитор\n")
	m.print("  6. Сохранить и применить\n")
	m.print("  0. Выход\n\n")
}
...
func (m *serverMenu) clientLine(token string) {
	m.printf("  адрес: %s\n  токен: %s\n", m.gwHint(), token)
	if m.cfg.TransportKey != "" {
		m.printf("  транспортный ключ: %s\n", m.cfg.TransportKey)
	}
}
...
func (m *serverMenu) setListen(kind string) bool {
	...
	m.dirty = true
	if kind == "udp" && m.cfg.TransportKey == "" {
		m.print("для udp нужен транспортный ключ\n")
		yes, ok := m.confirm("сгенерировать?")
		if !ok || !yes {
			return ok
		}
		return m.genKey()
...
// transportKey — общий ключ шлюза: им шифруются датаграммы udp, пользователей
// он не различает.
func (m *serverMenu) transportKey() bool {
	for {
		m.print(clearScr)
		m.print("Транспортный ключ\n\n")
		m.print("  общий для всех пользователей шлюза, нужен для udp\n")
		m.printf("  ключ: %s\n\n", orUnset(mask(m.cfg.TransportKey)))
		m.print("  1. Сгенерировать новый\n")
		m.print("  2. Вписать свой\n")
		m.print("  3. Показать целиком\n")
		m.print("  0. Назад\n\n")
...
func (m *serverMenu) genKey() bool {
	if m.cfg.TransportKey != "" {
		yes, ok := m.confirm("сменить ключ? клиентам придётся вписать новый")
		if !ok || !yes {
			return ok
		}
	}
	key, err := config.NewTransportKey()
	if err != nil {
		return m.fail(err)
	}
	m.cfg.TransportKey = key
	m.dirty = true
	m.printf("\nтранспортный ключ: %s\n  впишите его всем клиентам\n", key)
	return m.pause()
}

func (m *serverMenu) setKey() bool {
	key, ok := m.input("ключ (пусто — отмена): ", checkKey)
	if !ok || key == "" {
		return ok
	}
	m.cfg.TransportKey = key
...
func (m *serverMenu) showKey() bool {
	m.printf("\n%s\n", orUnset(m.cfg.TransportKey))
	return m.pause()
}
...
func checkServer(c config.Config) error {
	...
	if _, udp := c.Listen["udp"]; udp && c.TransportKey == "" {
		return errors.New("sunfire: для udp нужен транспортный ключ")
	}
...
func (m *clientMenu) servers() bool {
	for {
		...
		for i, s := range m.cfg.Servers {
			line := fmt.Sprintf("  %2d. %-16s %-24s %s", i+1, s.Name, s.GW, mask(s.Token))
			if s.TransportKey == "" {
				line += " · только tcp"
			}
			m.print(line + "\n")
...
func (m *clientMenu) addServer() bool {
	...
	key, ok := m.input("транспортный ключ (пусто — только tcp): ", checkKey)
	if !ok {
		return false
	}
	m.cfg.Servers = append(m.cfg.Servers, clientconf.Server{Name: name, GW: gw, Token: token, TransportKey: key})
	m.dirty = true
	return true
}
...
func (m *clientMenu) editServer() bool {
	...
	key, ok := m.input(fmt.Sprintf("транспортный ключ [%s]: ", orUnset(mask(s.TransportKey))), checkKey)
	if !ok {
		return false
	}
	if name != "" {
...
// apply просит службу перечитать конфиг. Пользователи, лимиты и привязки
// применяются без разрыва соединений; адреса и транспортный ключ — только рестартом.
func (u ui) apply(unit string) {
	if _, err := exec.LookPath("systemctl"); err != nil {
		u.printf("systemctl не найден, перечитайте конфиг сами: systemctl reload %s\n", unit)
		return
	}
	out, err := exec.Command("systemctl", "reload", unit).CombinedOutput()
	if err == nil {
		u.printf("служба %s перечитала конфиг, соединения не разорваны\n", unit)
		return
	}
	...
	u.printf("перечитать не вышло (%s), перезапускаю\n", msg)
	u.restart(unit)
}
...
func checkKey(s string) error {
	if utf8.RuneCountInString(s) < minKeyLen {
		return fmt.Errorf("sunfire: транспортный ключ короче %d символов", minKeyLen)
	}
	return nil
}
