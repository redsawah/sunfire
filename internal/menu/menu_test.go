...
func TestChecks(t *testing.T) {
	cases := []struct {
		...
		{"key", checkKey, "0123456789abcdef", true},
		{"key", checkKey, "0123456789abcde", false},
...
func TestCheckServer(t *testing.T) {
	...
	c.TransportKey = ""
	if err := checkServer(c); err == nil {
		t.Fatal("udp без транспортного ключа прошёл проверку")
	}
	delete(c.Listen, "udp")
	if err := checkServer(c); err != nil {
		t.Fatalf("без udp ключ не нужен: %v", err)
	}
}

func TestServerTransportKey(t *testing.T) {
	u, out := script(
		"1", "д", "", // сменить ключ на новый
		"3", "", // показать целиком
		"2", "коротко", "0123456789abcdefghij", // вписать свой, сперва короткий
		"0",
	)
	...
	if m.cfg.TransportKey != "0123456789abcdefghij" {
		t.Fatalf("ключ %q", m.cfg.TransportKey)
	}
	if !m.dirty {
		t.Error("изменения не отмечены")
	}
	if !strings.Contains(out.String(), "короче") {
		t.Error("короткий ключ прошёл молча")
	}
}

// Включили udp — меню предлагает сгенерировать недостающий ключ.
func TestServerListenUDPKey(t *testing.T) {
	u, _ := script(":9001", "д", "")
	m := &serverMenu{ui: u, cfg: config.Config{Listen: map[string]string{}}}
	if !m.setListen("udp") {
		t.Fatal("оборвалось")
	}
	if len(m.cfg.TransportKey) < minKeyLen {
		t.Fatalf("ключ не сгенерировался: %q", m.cfg.TransportKey)
	}
}
...
func TestClientServers(t *testing.T) {
	u, _ := script(
		"1", "дом", "гв", "1.2.3.4:9000", "коротко", "0123456789abcdef", "", // адрес и токен с ошибкой, без ключа
		"1", "запас", "5.6.7.8:9000", "fedcba9876543210", "0123456789abcdefghij",
	...
	if m.cfg.Servers[1].GW != "1.2.3.4:9000" || m.cfg.Servers[1].TransportKey != "" {
		t.Fatalf("сервер не тот: %+v", m.cfg.Servers[1])
	}
	if m.cfg.Servers[0].TransportKey != "0123456789abcdefghij" {
		t.Fatalf("транспортный ключ не взялся: %+v", m.cfg.Servers[0])
	}
}
