...
type Options struct {
	...
	Token string
	// TransportKey — транспортный ключ шлюза. Пусто — остаётся только tcp.
	TransportKey string
	Transports   []string      // если пусто — все доступные с этим ключом
...
func Run(opts Options) []Result {
	...
	if len(names) == 0 {
		names = transport.Available(opts.TransportKey)
	}
	if opts.Duration <= 0 {
...
func measure(name string, opts Options) Result {
	...
	if transport.NeedsKey(name) && opts.TransportKey == "" {
		return Result{Transport: name, Err: errors.New("sunfire: не задан транспортный ключ")}
	}
	raw, err := t.Dial(addr, opts.TransportKey, opts.Timeout)
	if err != nil {
		return Result{Transport: name, Err: err}
	}
