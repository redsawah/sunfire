...
const (
	...
	// буфер чтения — больше самого крупного кадра
	readBufSize = 32 * 1024
...
type Conn struct {
	...
	decNonce  []byte
	readBuf   []byte
	wbuf      []byte // кадр целиком перед отправкой
	pbuf      []byte // полезное вместе с набивкой
	wroteHead bool
}
...
func (c *Conn) sealFrame(chunk []byte, pad int) error {
	...
	lenCipher := c.enc.Seal(c.wbuf[:0], c.encNonce, hdr[:], nil)
	incNonce(c.encNonce)
	...
	if pad == 0 {
		...
		if cap(c.pbuf) < len(chunk)+pad {
			c.pbuf = make([]byte, len(chunk)+pad)
		}
		body := c.pbuf[:len(chunk)+pad]
		copy(body, chunk)
