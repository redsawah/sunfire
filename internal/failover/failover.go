...
// Одна неудача — ещё не отказ сервера. На нагруженном плече пакеты теряются, и
// отдельное подключение не укладывается в таймаут при живом сервере. Уходим
// только если подряд не удалось несколько подключений за короткое окно, а любое
// удачное подключение счётчик обнуляет.
const (
	failThreshold = 3
	failWindow    = 20 * time.Second
)
...
type Pool struct {
	...
	fails    int       // неудач подряд на текущем сервере
	firstBad time.Time // когда началась текущая череда неудач
}

// OK сообщает, что подключение к текущему серверу удалось: череда неудач прервана.
func (p *Pool) OK() {
	p.mu.Lock()
	p.fails = 0
	p.mu.Unlock()
...
func (p *Pool) failed(err error, timeout time.Duration) (clientconf.Server, error) {
	if p.cur >= 0 {
		now := p.now()
		if p.fails == 0 || now.Sub(p.firstBad) > failWindow {
			p.fails, p.firstBad = 0, now
		}
		p.fails++
		if p.fails < failThreshold {
			// одиночная потеря пакета — сервер менять рано
			return p.servers[p.cur], nil
		}
		p.until[p.cur] = p.now().Add(cooldown)
		p.logf("сервер %s отвалился: %d неудач подряд, последняя: %v",
			p.servers[p.cur].Name, p.fails, err)
		p.fails = 0
	}
	return p.pick(timeout)
}
